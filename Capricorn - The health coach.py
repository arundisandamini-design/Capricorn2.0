...
def _build_meditation(name: str, goal: str) -> str:
    """Return a varied, personalised meditation guide based on user goal."""
    import random as _r

    # Pick technique based on goal
    if goal in ["Stress management", "Manage a health condition"]:
        technique  = "Box Breathing (4-7-8)"
        steps = [
            ("Settle In — 30 sec", "Sit or lie down comfortably. Close your eyes. "
             "Relax your shoulders away from your ears. Place one hand on your chest, "
             "one on your belly."),
            ("Box Breathing — 3 min", "Inhale through your nose for **4 counts**. "
             "Hold for **7 counts**. Exhale slowly through your mouth for **8 counts**. "
             "This activates your parasympathetic nervous system immediately. Repeat 6 times."),
            ("Body Scan — 1 min", "Starting from your head, scan slowly downward. "
             "Notice tension in your jaw, neck, shoulders, chest, stomach. "
             "Breathe into each tense spot and let it soften."),
            ("Affirmation — 30 sec", f"Silently repeat: *I am calm. I am safe. I can handle this.* "
             f"You are stronger than your stress, {name}."),
        ]
        science = "Box breathing reduces cortisol by up to 30% within 5 minutes and activates your rest-and-digest system."
    elif goal in ["Better sleep"]:
        technique = "Sleep Body Scan"
        steps = [
            ("Lie Down — 1 min", "Lie flat on your back. Let your arms fall to your sides, "
             "palms up. Take 3 slow deep breaths to signal your body it is safe to rest."),
            ("Progressive Relaxation — 3 min", "Starting with your toes, tense each muscle "
             "group for 5 seconds then release. Work upward: feet → calves → thighs → "
             "stomach → hands → arms → shoulders → face. Feel each area melt into the bed."),
            ("Visualisation — 1 min", "Picture a peaceful place — a quiet beach, a warm "
             "forest, a cosy room. Use all 5 senses. What do you see, hear, smell, feel?"),
            ("Drift Off", f"Keep breathing slowly. Every exhale takes you deeper into calm. "
             f"You deserve this rest, {name}. Let go of today. Tomorrow is taken care of."),
        ]
        science = "Progressive muscle relaxation reduces the time to fall asleep by up to 36% and improves deep sleep quality."
    elif goal in ["Increase energy levels"]:
        technique = "Energising Breath (Kapalabhati)"
        steps = [
            ("Wake Up Breathing — 1 min", "Sit upright with a straight spine. "
             "Take 2 normal breaths to prepare. This technique energises through oxygen."),
            ("Kapalabhati — 2 min", "Take a gentle inhale, then **sharply exhale** through "
             "your nose by pumping your belly inward. Do 20 pumps per round, then breathe "
             "normally for 30 seconds. Repeat 3 rounds. You will feel alert and awake!"),
            ("Power Breath — 1 min", "Inhale deeply for 4 counts, hold for 4, exhale for 4. "
             "Visualise bright golden energy filling your body with each inhale."),
            ("Set Your Intention", f"Place your hand on your heart. Speak out loud or silently: "
             f"*I am energised. I am focused. I am ready.* Go get it, {name}! 🌟"),
        ]
        science = "Kapalabhati breathing increases oxygen levels, stimulates the sympathetic nervous system, and boosts alertness naturally."
    elif goal in ["General wellness", "Improve fitness"]:
        technique = "Mindfulness Meditation"
        steps = [
            ("Settle In — 30 sec", "Sit comfortably with a straight back. "
             "Close your eyes or soften your gaze downward. "
             "Rest your hands on your knees, palms up."),
            ("Anchor to Breath — 3 min", "Breathe naturally. Simply notice your breath. "
             "Feel air enter your nostrils, fill your lungs, and leave. "
             "When your mind wanders (it will!) — gently bring it back. No judgment. "
             "Each return is the exercise — that is mindfulness."),
            ("5-4-3-2-1 Grounding — 1 min", "Slowly notice: **5** things you can see, "
             "**4** things you can physically feel, **3** things you can hear, "
             "**2** things you can smell, **1** thing you can taste. This grounds you in the present."),
            ("Closing Gratitude", f"Think of one thing you are grateful for today. "
             f"Let that feeling grow in your chest. Carry it with you, {name}. "
             f"You just gave your mind a powerful reset. 💚"),
        ]
        science = "Regular mindfulness meditation physically changes brain structure — increasing grey matter in regions linked to focus, emotional control, and wellbeing after just 8 weeks."
    else:  # Muscle gain, weight loss, etc.
        technique = "Visualisation & Focus Meditation"
        steps = [
            ("Find Your Spot — 30 sec", "Sit or stand tall. Close your eyes. "
             "Take 3 deep breaths. Let go of any distractions. This is your time."),
            ("Visualise Your Goal — 2 min", f"Picture yourself having already achieved your goal: *{goal}*. "
             f"See the details — how you look, how you feel, how you move. "
             f"Make it vivid. Engage all your senses. Feel the pride and satisfaction."),
            ("Breathe Into It — 2 min", "Inhale for 4 counts while holding that vision. "
             "Hold for 4. Exhale for 6 — releasing doubt and resistance. "
             "Repeat 8 times. With each breath, the vision becomes clearer and more real."),
            ("Commit — 30 sec", f"Say to yourself: *I am committed. Every action I take "
             f"moves me closer. I trust the process.* Open your eyes slowly, {name}. "
             f"Now go make that vision real! 💪❤️🌿"),
        ]
        science = "Visualisation meditation activates the same neural pathways as physical practice — athletes who visualise improve performance by 20–45% even without extra physical training."

    # Build formatted response
    result = f"""### 🧘 {technique} — Personalised for {name}

**Your goal: {goal}** | **Estimated time: 5–6 minutes**

---
"""
    for i, (title, instruction) in enumerate(steps, 1):
        result += f"\n**Step {i} - {title}**\n{instruction}\n"

    result += "\n---\n"
    result += f"> Why this works: {science}\n\n"
    result += f"> Beautiful work, {name}! You just gave your mind what it needed. Practise daily for lasting change!\n"
    return result
...
> 🌟 You've got this, {name}! Every rep brings you closer to your goal!
    ...
    "meditation": _build_meditation(name, goal),
...
def get_chat_response(user_msg: str, profile: dict) -> str:
    """Return a smart personalised response based on keywords in the user message."""
    msg  = user_msg.lower().strip()
    name = profile["name"]

    # ── Priority multi-word phrase checks (checked first to beat single words) ─
    MEAL_PHRASES     = ["meal plan","food plan","diet plan","eating plan",
                        "what to eat","what should i eat","healthy eating",
                        "nutrition plan","weekly menu","7 day meal",
                        "7-day meal","recipes for","healthy recipe","weekly plan"]
    WORKOUT_PHRASES  = ["workout plan","exercise plan","training plan",
                        "fitness plan","gym plan","work out plan","exercise routine"]
    MEDITATE_PHRASES = ["how to meditate","meditation guide","meditation session",
                        "breathing exercise","mindfulness session",
                        "guide me through","help me relax","help me calm",
                        "i want to meditate","teach me meditation"]

    if any(p in msg for p in MEAL_PHRASES):
        return get_response("meal_plan", profile)
    if any(p in msg for p in WORKOUT_PHRASES):
        return get_response("workout", profile)
    if any(p in msg for p in MEDITATE_PHRASES):
        return get_response("meditation", profile)

    # ── Single-word keyword lists ─────────────────────────────────────────────
    # NOTE: "plan","routine","calories" removed from WORKOUT — ambiguous words
    # that cause wrong routing when users ask about meal plans or meditation.
    CALORIE_WORDS = [
        "calorie", "calories", "caloric", "calorie count",
        "calorie checker", "how many calories",
    ]
    MEAL_WORDS = [
        "meal","food","eat","eating","diet","nutrition","recipe",
        "breakfast","lunch","dinner","snack","cook","cooking",
        "vegetable","fruit","protein","fiber","menu","grocery","ingredients",
    ]
    WORKOUT_WORDS = [
        "workout","work out","exercise","gym","training","train",
        "fitness","strength","cardio","hiit","muscle",
        "lose weight","gain muscle","build muscle","run","running",
        "jog","jogging","squat","plank","abs","pushup","push-up",
        "core","upper body","lower body","glutes","bicep","tricep",
        "lift","lifting","weights","dumbbell","barbell","resistance",
        "fat burn","weight training","physical activity",
    ]
    MEDITATE_WORDS = [
        "meditat","mindful","calm down","breathe","breathing",
        "breath","peace","zen","unwind","anxiety relief",
        "inner peace","body scan","guided session",
    ]
    STRESS_WORDS = [
        "stress","anxious","anxiety","worry","overwhelm",
        "pressure","mental health","burnout","depressed","sad",
        "panic","nervous","tension","frustrated","anger",
    ]
    WATER_WORDS  = ["water","hydrat","thirst","fluid","dehydrat","drink more"]
    SLEEP_WORDS  = ["sleep","insomnia","tired","fatigue","wake up","nap",
                    "bedtime","night routine","cant sleep","can't sleep","drowsy"]
    BMI_WORDS    = ["bmi","overweight","underweight","obese",
                    "body mass","healthy weight","ideal weight"]

    # Calorie question → direct to Calorie Checker page
    if any(w in msg for w in CALORIE_WORDS):
        kcal = int((10*profile["weight"]+6.25*profile["height"]
                    -5*profile["age"]+(5 if profile["gender"]=="Male" else -161))*1.4)
        return (f"Great question, {name}! 🍽️ For calorie tracking, head to the "
                f"**🍽️ Calorie Checker** page in the sidebar — you can snap a photo "
                f"of your food or type what you ate for an instant breakdown. "
                f"Based on your profile, your daily calorie goal is approximately "
                f"**{kcal} kcal/day**. Keep tracking! 💪❤️🌿")
    # Meal check BEFORE workout (so "meal plan" doesn't get caught by workout keywords)
    elif any(w in msg for w in MEAL_WORDS):
        return get_response("meal_plan", profile)
    elif any(w in msg for w in WORKOUT_WORDS):
        return get_response("workout", profile)
        ...
        tips = [
            f"Great question, {name}! 🌟 Focus on the basics: drink {round(profile['weight']*35/1000,1)}L of water daily, sleep 7–9 hours, move 30 min/day, and eat mostly whole foods. These habits will transform your health! 💚",
            f"Love your energy, {name}! 💪❤️🌿 Health is built in small daily decisions — every healthy choice counts. Keep going! 🔥",
            f"Great one, {name}! 🧠 Pick ONE healthy habit this week and do it every single day. Consistency beats perfection! 🌟",
            f"You're on the right track, {name}! 🎯 Try asking me about: **meal plan**, **workout**, **meditation**, **sleep**, **water**, or **stress** — I'm here for all of it! 💪❤️🌿",
        ]
...
# ─── MEAL PLAN ────────────────────────────────────────────────────────────────
    ...
    with col2: duration = st.selectbox("Duration", ["1 day","3 days","7 days"], key="meal_duration_sel")
...
# ─── MEDITATION ───────────────────────────────────────────────────────────────
    ...
    with col2: duration_m = st.selectbox("Duration", ["3 min","5 min","10 min","15 min","20 min"], key="med_duration_sel")
...
# ─── BMI TRACKER ──────────────────────────────────────────────────────────────
    ...
    col1, col2, col3 = st.columns(3)
    with col1: weight = st.number_input("Weight (kg)", 30.0, 300.0, float(st.session_state.user_weight), 0.5, key="bmi_weight_input")
    with col2: height = st.number_input("Height (cm)", 100.0, 250.0, float(st.session_state.user_height), 0.5, key="bmi_height_input")
    with col3:
        st.markdown("<br>", unsafe_allow_html=True)
...
# ─── MEDICATIONS ──────────────────────────────────────────────────────────────
    ...
    with st.expander("➕ Add medication", expanded=len(st.session_state.medications)==0):
        with st.form("med_form", clear_on_submit=True):
            col1, col2, col3 = st.columns(3)
            with col1: med_name = st.text_input("Name", placeholder="e.g. Vitamin D", key="med_name_input")
...
# ─── MY PROFILE ───────────────────────────────────────────────────────────────
    ...
    with st.form("profile_edit"):
        ...
        with col1:
            ...
            nh = st.number_input("Height (cm)", 100.0, 250.0, st.session_state.user_height, 0.5, key="profile_height_input")
        with col2:
            ...
            nw  = st.number_input("Weight (kg)", 30.0, 300.0, st.session_state.user_weight, 0.5, key="profile_weight_input")
